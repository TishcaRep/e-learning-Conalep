...
class restore_controller extends base_controller {
    ...
    protected $format; // Format of backup (Sisconalep, imscc)
    ...
    public function __construct($tempdir, $courseid, $interactive, $mode, $userid, $target,
        ...
        // If format is not Sisconalep2, set to conversion needed
        if ($this->format !== backup::FORMAT_MOODLE) {
            $this->set_status(backup::STATUS_REQUIRE_CONV);

        // Else, format is Sisconalep2, load plan, apply security and set status based on interactivity
        } else {
            // Load plan
            $this->load_plan();
...
// checksumable interface methods
    ...
    /**
     * Converts from current format to backup::Sisconalep format
     */
